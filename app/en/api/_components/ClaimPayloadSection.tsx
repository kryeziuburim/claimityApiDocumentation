...
export const CLAIM_PAYLOADS: ClaimPayloadMeta[] = [
  {
    key: "vehicle",
    navTitle: "Vehicle Appraiser",
    anchorId: "payloads-vehicle",
    label: "Vehicle Appraiser",
    schemaPath: "/assets/schemas/vehicle-claim.schema.json",
    badgeLabel: "Vehicle Appraiser",
  },
  {
    key: "appraiser",
    navTitle: "Appraiser",
    ...
    badgeLabel: "Appraiser",
  },
  {
    key: "fraud",
    navTitle: "Fraud Prevention",
    anchorId: "payloads-fraud",
    label: "Fraud Prevention",
    schemaPath: "/assets/schemas/fraud-claim.schema.json",
    badgeLabel: "Fraud Prevention",
  },
  {
    key: "special",
    navTitle: "Special Appraisals",
    anchorId: "payloads-special",
    label: "Special Appraisals",
    schemaPath: "/assets/schemas/special-claim.schema.json",
    badgeLabel: "Special Appraisals",
  },
]
...
export function ClaimPayloadSection({
  ...
  useEffect(() => {
    ...
    CLAIM_PAYLOADS.forEach((payload) => {
      fetch(payload.schemaPath)
        .then((res) => {
          if (!res.ok) throw new Error(`Schema ${payload.label} could not be loaded.`)
          return res.json()
        })
        .then((json) => {
          if (cancelled) return
  ...
  const handleCopyJson = useCallback(
    async (payloadKey: string, payloadLabel: string, json: string) => {
      ...
      toast({
        title: success ? "JSON copied" : "Copy failed",
        description: success
          ? `${payloadLabel} Payload copied to clipboard.`
          : "Please copy the JSON content manually.",
  ...
  const handleInsertExample = useCallback(() => {
    ...
    if (!schema) {
      toast({
        ...
        description: "The schema for this category could not be loaded yet.",
        variant: "destructive",
      })
      return
    }
    ...
    if (!example) {
      toast({
        ...
        description: "The schema does not contain an example JSON for this category.",
        variant: "destructive",
      })
      return
    }
    ...
    toast({
      ...
      description: `${meta?.label ?? "Category"} example copied to input field.`,
    })
  ...
  const handleValidatePayload = useCallback(async () => {
    ...
    if (!trimmed) {
      setValidationState({ status: "error", message: "Please enter a Payload JSON." })
      return
    }
    ...
    try {
      ...
      setValidationState({
        status: "error",
        message: `JSON could not be parsed: ${errorMessage}`,
      })
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setValidationState({
          ...
          message: detail ? String(detail) : `Request failed (HTTP ${response.status}).`,
  ...
  const validationPanel = (() => {
      ...
      return (
        <div className="space-y-4">
          ...
          >
            ...
            <AlertTitle>{data.valid ? "Payload is valid" : "Payload violates validation rules"}</AlertTitle>
            <AlertDescription className="space-y-2 text-sm">
              <p>
                {data.valid
                  ? "The validator found no schema deviations."
                  : "At least one rule of the schema was violated. See details below."}
              </p>
          ...
          {errorDetails ?? (!data.valid ? (
            <p className="rounded-2xl border border-border/60 bg-muted/20 p-3 text-sm text-muted-foreground">
              The API reported an invalid payload but returned no error entries.
            </p>
          ) : null)}
        </div>
      )
    ...
    return (
      <div className="space-y-3 text-sm text-muted-foreground">
        <p>The validation API response appears here.</p>
        <ol className="list-decimal space-y-1 pl-4 text-xs">
          <li>Select category</li>
          <li>Insert Payload JSON or use example</li>
  ...
  return (
    <div id="claim-payloads" className="space-y-6">
      <div>
        <h2 className="mb-4 text-3xl font-bold tracking-tight text-balance">Case Structure & Validation</h2>
                ...
                <div className="space-y-2 sm:flex sm:flex-wrap sm:items-start sm:justify-between sm:gap-4">
                  <div className="sm:max-w-[75%]">
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">
                      This structure is intended for payloads for the category {payload.badgeLabel}.
                    </p>
                  </div>
                </div>
                    ...
                    <div className="space-y-6">
                      <div className="grid gap-4 sm:gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,1.1fr)]">
                        <Card className="hidden min-w-0 border-border/60 md:block">
                          <CardHeader>
                            <CardTitle className="text-base font-semibold">Context & Hints</CardTitle>
                          </CardHeader>
                          <CardContent className="space-y-4 text-sm text-muted-foreground">
                          <p>
                            The payload category <span className="font-medium text-foreground">{payload.label}</span> includes
                            {" "}
                            {stats?.totalFields ?? 0} documented fields as well as {stats?.requiredFields ?? 0} required fields.
                          </p>
                          <div className="grid gap-3">
                            <InlineHint icon={Info} label="Format Requirements" value={formatHints.length ? `${formatHints.length} specifics` : "No special requirements"} />
                            <InlineHint icon={ShieldCheck} label="Rules & Dependencies" value={ruleCount ? `${ruleCount} defined rules` : "No additional rules"} />
                          </div>
                        </CardContent>
                      </Card>
                        ...
                        <Card className="min-w-0 border-border/60 bg-muted/40">
                          <CardHeader>
                            <CardTitle className="text-base font-semibold">Actions & Links</CardTitle>
                          </CardHeader>
                        <CardContent className="space-y-4">
                          ...
                          <p className="text-sm text-muted-foreground">
                            All schemas are available as JSON files and can be imported directly into validation tools.
                          </p>
                        </CardContent>
                      </Card>
                    </div>
                              ...
                              <div className="relative">
                                <ScrollArea className="h-[360px] max-w-full rounded-2xl border border-border/60">
                                  <button
                                    ...
                                    aria-label={`Copy ${payload.label} Example JSON`}
                                  >
                        ...
                        <AccordionItem value="formats" className="border-border/40 px-4 sm:px-6">
                          ...
                          <AccordionContent className="space-y-3 px-1">
                            {formatHints.length ? (
                              ...
                              <p className="text-sm text-muted-foreground">No special format restrictions documented.</p>
                            )}
                          </AccordionContent>
                        </AccordionItem>
                        ...
                        <AccordionItem value="rules" className="border-border/40 px-4 sm:px-6">
                          ...
                          <AccordionContent className="space-y-6 px-1">
                            {ruleGroups.length ? (
                                    ...
                                    {group.rules.map((rule, index) => (
                                      ...
                                      >
                                        <p className="text-sm font-semibold text-foreground">
                                          <RuleText text={rule.type === "if" ? `If ${rule.condition}, then:` : rule.condition} />
                                        </p>
                              ...
                              <p className="text-sm text-muted-foreground">No additional validation rules defined for this schema.</p>
                            )}
                          </AccordionContent>
                        </AccordionItem>
      ...
      >
        <div className="space-y-2">
          <h3 className="text-2xl font-semibold tracking-tight">Test PayloadJson directly</h3>
          <p className="text-sm text-muted-foreground">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="payload-category-select">Category</Label>
...
function buildClaimRuleGroups(schema: any): ClaimRuleGroup[] {
  ...
  conditionalGroups.forEach((rules, field) => {
    groups.push({
      key: field,
      title: field === GENERIC_TRIGGER ? "Further conditions" : `Depending on \`${field}\``,
      rules,
    })
  })
  if (exclusions.length) {
    groups.push({ key: "exclusions", title: "Combinations that are not allowed", rules: exclusions })
  }
  return groups
}
...
function buildDriverAtIncidentSummary(schema: any): ClaimRule | null {
  ...
  return details.length
    ? {
        type: "if",
        condition: "`driverAtIncident` is set",
        details,
      }
    : null
}
...
function describeDriverYesLimit(
  ...
  const optionalNote = !minContains || minContains <= 0 ? " (or none)" : ""
  const limitText = maxContains === 1 ? "at most one person" : `at most ${maxContains} persons`
  return `${contextLabel}: In ${objectName} and ${arrayName}[] ${limitText} may be driverAtIncident = "yes"${optionalNote}.`
}
...
function describeFormatDetail(schema: any): string | null {
  if (schema.format) {
    switch (schema.format) {
      ...
      case "date-time":
        return "Date & Time in ISO format YYYY-MM-DDTHH:MM:SSZ"
      case "time":
  ...
  if (schema.pattern) {
    if (schema.pattern === "^([01]\\d|2[0-3]):([0-5]\\d)$") {
      return "Time field HH:MM (24h)"
    }
    if (schema.pattern === "^-?\\d+(\\.\\d+)?$") {
      return "Numeric string (optional decimal places, dot as separator)"
    }
    return `Must match Regex ${schema.pattern}`
  }

  return null
}

const CONDITION_FALLBACK = "a special condition is met"
...
function describeRuleCondition(condition: any, parentPath = ""): string {
  ...
  if (Array.isArray(condition.required) && condition.required.length && !condition.properties) {
    condition.required.forEach((field: string) => {
      ...
      parts.push(`the field \`${fieldPath}\` is present`)
    })
  }
  ...
  return parts.length ? parts.join(" and ") : CONDITION_FALLBACK
}
...
function describeArrayContains(path: string, schema: any): string {
  ...
  if (minContains !== undefined && maxContains !== undefined && minContains === maxContains) {
    descriptor = minContains === 1 ? "exactly one" : `exactly ${minContains}`
  } else if (minContains !== undefined && maxContains !== undefined) {
    const minPart = minContains > 0 ? (minContains === 1 ? "at least one" : `at least ${minContains}`) : undefined
    const maxPart = maxContains === 1 ? "at most one" : `at most ${maxContains}`
    descriptor = [minPart, maxPart].filter(Boolean).join(" and ")
  } else if (minContains !== undefined) {
    descriptor = minContains <= 0 ? "at least one" : minContains === 1 ? "at least one" : `at least ${minContains}`
  } else if (maxContains !== undefined) {
    descriptor = maxContains === 1 ? "at most one" : `at most ${maxContains}`
  } else {
    descriptor = "at least one"
  }

  const elementLabel = descriptor === "exactly one" || descriptor === "at least one" || descriptor === "at most one" ? "element" : "elements"
  const requirement = describeRuleCondition(schema.contains, `${path}[]`)
  const useMay = descriptor.startsWith("at most")
  const verb = useMay ? "may" : "must"
  const optionalNote = useMay && (minContains === undefined || minContains <= 0) ? " (or none)" : ""

  return `${path}[] ${verb} contain ${descriptor} ${elementLabel}${optionalNote} where ${requirement}`
}
...
function describeRuleConsequences(thenClause: any): string[] {
  if (!thenClause) return ["Additional requirements apply."]
  const lines: string[] = []
  if (Array.isArray(thenClause.required) && thenClause.required.length) {
    lines.push(`Required fields: ${formatFieldList(thenClause.required)}`)
  }
  ...
  return lines.length ? lines : ["Additional requirements apply."]
}
...
function describeConsequenceFacts(path: string, schema: any): string[] {
  ...
  if (schema.const !== undefined) {
    facts.push(`\`${path}\` must be \`${formatSchemaValue(schema.const)}\``)
  } else if (Array.isArray(schema.enum) && schema.enum.length) {
    facts.push(`\`${path}\` must be one of ${formatValueList(schema.enum)}`)
  }

  if (Array.isArray(schema.required) && schema.required.length) {
    facts.push(`\`${path}\` requires ${formatFieldList(schema.required)}`)
  }
...
function describeRuleNot(node: any): string {
  ...
  if (node?.allOf) {
    const segments = node.allOf
      ...
      .filter(Boolean)
    if (segments.length) {
      return `Combination of ${segments.join(" + ")}`
    }
  }
...
function describeDriverConflict(node: any): string | null {
  ...
  return `"yes" must not occur simultaneously in ${formatDriverSegment(conflict.objectSegment)} and ${formatDriverSegment(conflict.arraySegment)}.`
}
